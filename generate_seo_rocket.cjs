...
// Helper: Extract location from title/content
function extractLocation(title, content) {
  const locations = ['Miami', 'Fort Lauderdale', 'Palm Beach', 'South Florida', 'NYC', 'New York', 
                     'Hudson Valley', 'Manhattan', 'Brooklyn', 'Queens', 'Sarasota', 'Key West',
                     'Coral Gables', 'Brickell', 'Wynwood', 'Coconut Grove', 'Cold Spring',
                     'Greensboro', 'North Carolina', 'Lake Como', 'Italy', 'Davie'];
...
// Helper: Extract venue from title/content
function extractVenue(title, content) {
  const venueKeywords = ['farm', 'barn', 'hotel', 'resort', 'villa', 'estate', 'mansion', 
                         'beach', 'park', 'garden', 'restaurant', 'venue', 'hall'];
  
  const text = (title + ' ' + content).toLowerCase();
  
  // Look for venue names (capitalized words near venue keywords)
  const venueRegex = /([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+(Farm|Barn|Hotel|Resort|Villa|Estate|Park|Restaurant)/g;
  const matches = (title + ' ' + content).match(venueRegex);
  
  return matches ? matches.slice(0, 2) : [];
}

// Helper: Extract names from title
function extractNames(title) {
  // Look for pattern "Name + Name" or "Name & Name"
...
// Helper: Generate SEO keywords
function generateKeywords(title, category, locations, names) {
  const keywords = [
    ...
    ...names.map(name => name + ' ' + category.toLowerCase()),
...
function generatePostHTML(post, allPosts) {
  ...
  const venues = extractVenue(title, post.content.rendered);
  const names = extractNames(title);
  const keywords = generateKeywords(title, category, locations, names);
  ...
  const relatedPosts = allPosts
    .filter(p => p.slug !== slug && getCategory(cleanTitle(p.title.rendered)) === category)
    .slice(0, 3);
...
<head>
    ...
    <meta name="author" content="Acromatico Photography">
    ...
    <meta property="og:site_name" content="Acromatico Photography">
    <meta property="article:published_time" content="${isoDate}">
    <meta property="article:author" content="Acromatico">
    <meta property="article:section" content="${category}">
    
    <!-- Twitter Card -->
    <meta name="twitter:card" content="summary_large_image">
    <meta name="twitter:site" content="@acromatico">
    ...
    <script type="application/ld+json">
    {
      ...
      "itemListElement": [
        ...
        },
        {
          ...
          "position": 3,
          "name": "${category}",
          "item": "https://acromatico.com/blog?filter=${category.toLowerCase()}"
        },
        {
          "@type": "ListItem",
          "position": 4,
    ...
    ${allImages.length > 0 ? `<!-- ImageObject Schema for primary image -->
    <script type="application/ld+json">
    {
      "@context": "https://schema.org",
      "@type": "ImageObject",
...
<body>
    <nav>
        <div class="nav-content">
            <a href="/">
                <img src="/static/acromatico-logo-dark.png" alt="Acromatico Photography - ${locations.join(', ')}" class="logo-img">
            </a>
            <div class="nav-links">
    ...
    <article itemscope itemtype="https://schema.org/Article">
        <section class="hero">
            <div class="hero-content">
                ...
                <p class="post-meta">
                    <time itemprop="datePublished" datetime="${isoDate}">${date}</time>
                ...
                <section class="gallery">
                    <div class="gallery-grid">
                        ${allImages.map((img, idx) => `
                            <div class="gallery-item" itemprop="image" itemscope itemtype="https://schema.org/ImageObject">
                                <img src="${img}" 
                                     alt="${title} - Photo ${idx + 1}${locations.length > 0 ? ' in ' + locations[0] : ''}" 
                                     itemprop="contentUrl"
                                     loading="lazy">
                            </div>
                        `).join('')}
...
// Generate all posts
console.log(`🚀 Generating ${allPosts.length} blog posts with ROCKET FUEL SEO...`);
...
for (const post of allPosts) {
  try {
    const html = generatePostHTML(post, allPosts);
...
console.log(`\n🚀 SEO ROCKET FUEL ADDED:`);
console.log(`  ✅ Schema.org Article + BreadcrumbList + ImageObject`);
console.log(`  ✅ Location-based keywords extracted`);
console.log(`  ✅ Venue detection`);
console.log(`  ✅ Enhanced meta descriptions`);
console.log(`  ✅ Descriptive image alt tags`);
console.log(`  ✅ Semantic HTML with itemprop`);
console.log(`  ✅ Canonical URLs`);
console.log(`  ✅ Open Graph + Twitter Cards`);
console.log(`  ✅ Auto-generated SEO keywords`);
