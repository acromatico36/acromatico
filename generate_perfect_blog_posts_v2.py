#!/usr/bin/env python3
"""
ACROMATICO BLOG POST GENERATOR V2
COMPLETELY REPLACES OLD CONTENT - NO MIXING!
...
def generate_post_html(post_data: Dict, checklist_item: Dict) -> str:
    ...
    html = re.sub(
        r'<meta content="Celebrate 20 years.*?" property="og:description"/>',
    ...
    html = re.sub(
        r'<meta content="Celebrate 20 years.*?" property="twitter:description"/>',
        f'<meta content="{meta_description}" property="twitter:description"/>',
        html
    )
    ...
    # **CRITICAL FIX: COMPLETELY REMOVE ALL CONTENT BETWEEN INTRO AND GALLERY**
    # Find the content-container div
    content_start = html.find('<div class="content-container">')
    gallery_start = html.find('<div class="gallery-container">')
    
    if content_start != -1 and gallery_start != -1:
        # Extract everything BEFORE content-container
        before_content = html[:content_start]
        
        # Extract everything AFTER gallery (CTA, FAQ, footer, etc.)
        cta_start = html.find('<div class="cta-section mid-post">')
        after_gallery = html[cta_start:]
        
        # Build NEW content section with intro + 4 sections + gallery
        new_content = '<div class="content-container">\n'
        
        # Add intro paragraph
        new_content += f'<div class="section-text">\n<p>{content_text}</p>\n</div>\n\n'
        
        # Add 4 content sections with featured images from THIS post
        for i in range(4):
...
'''
        ...
        # Add gallery with THIS post's images
...
'''
        ...
        # Reassemble HTML with NEW content only
        html = before_content + new_content + after_gallery
...
def main():
    ...
    print("🚀 ACROMATICO BLOG POST GENERATOR V2")
    print("=" * 60)
